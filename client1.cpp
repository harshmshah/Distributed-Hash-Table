...
void CommunicateToServer1(map<string,string> & client1map) {
  string inp = "client1.txt";
  cout << "Please Enter Your Search (USC, UCLA etc.):" << endl;
  string search;
  cin >> search;
  string key = client1map[search];
  
  cout << "The Client 1 has received a request with search word " << search << " which maps to key " << key <<"." << endl;
  
  long status, socket_desc = -1;
  ...
  cout << "The Client 1 sends the request GET " << key_client << " to the Server 1 with port number " << get_port_num(p->ai_addr) << " and IP address " << ipstr <<"." << endl;
  cout << "The Client 1's port number is " << get_port_num(&myaddr) << " and the IP address is " << ipstr << endl;
  ...
  cout << " The Client 1 received the value " << msg_s1 << " from the Server 1 with port number " << server1_portnum << " and IP address " << get_IPaddress(their_addr) <<"." <<endl;
  cout << "The Client 1's port number is " << get_port_num(&myaddr) << " and the IP address is " << ipstr <<"." << endl;
  close(socket_desc);
...
int main(int argc, const char * argv[]) {
  map <string,string> client1map;
  
  // Build the hash map to store the key-value pairs in the memory.
  BuildClientMappings(client1map, "client1.txt", 12);
  // Call Client 1's function infinitely till the User commands Ctrl + C
  while (1) {
    CommunicateToServer1(client1map);
  }
  return 0;
}
